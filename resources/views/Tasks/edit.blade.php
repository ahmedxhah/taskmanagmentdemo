  ...
  <div class="p-4 bg-white rounded-lg shadow-xs">
    ...
    <form id="sample_form" action="{{route('task.update',$task->id)}}" method="POST">
      ...
      <label class="block mt-4 text-sm">
        ...
        <input name="title" value="{{$task->title}}" readonly type="text" class="form-control form-input block w-full mt-1 text-sm dark:border-gray-300 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray " placeholder="Enter task title">
      </label>
      <label class="block mt-4 text-sm">
        <span class="text-gray-700 dark:text-gray-400">
          Status
        </span>
        <select name="completed" class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
